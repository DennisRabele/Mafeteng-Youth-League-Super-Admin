...
def super_admin_dashboard(
    ...
    all_players = db.scalars(
        ...
        .where(Player.status != ApprovalStatus.REJECTED.value)
...
def team_admin_dashboard(
    ...
    players = db.scalars(
        select(Player)
        .options(
            selectinload(Player.team).selectinload(Team.category),
            selectinload(Player.documents),
        )
    ...
    transfer_history = db.scalars(
        ...
        .where(
            ...
            PlayerTransferRequest.status != ApprovalStatus.REJECTED.value,
        )
        .order_by(PlayerTransferRequest.transfer_id.desc())
    ).all()
